...
def vehicle_ownership_registry_worker(id: int):
    ...
    registry_session_maker = sessionmaker(bind=_engine.get("registry"), expire_on_commit=False)
    external_session_maker = sessionmaker(bind=_engine.get("external"), expire_on_commit=False)
    
    with registry_session_maker() as registry_session:
        task_record = None
        try:
            # Get the background task record
            task_record = registry_session.query(G2PQueBackgroundTask).filter_by(id=id).first()
            if not task_record:
            ...
            _logger.info(f"Processing batch {batch_number}/{total_batches} with params - "
            ...
            _logger.info(f"Successfully processed {records_processed} vehicle ownership records "
            ...
            if task_record:
                ...
                registry_session.commit()
            _logger.error(f"Vehicle ownership worker task failed for id: {id}, error: {str(e)}", exc_info=True)
            raise
...
def _process_vehicle_ownership_data(
    ...
    with external_session_maker() as external_session:
        ...
        _logger.info(f"Fetching RTA registration records with offset {page_offset}, limit {page_size}")
        ...
        for rta_record in results:
            try:
                ...
                existing_record = registry_session.query(G2PRegistryVehicleOwnership).filter_by(
                    unique_vehicle_id=rta_record.regnno
                ).first()
                
                if existing_record:
                    # Update existing record
                    _update_vehicle_ownership_record(existing_record, rta_record)
                    _logger.debug(f"Updated existing vehicle ownership record for vehicle: {rta_record.regnno}")
                else:
                    # Create new record
                    new_record = _create_vehicle_ownership_record(rta_record)
                    registry_session.add(new_record)
                    _logger.debug(f"Created new vehicle ownership record for vehicle: {rta_record.regnno}")
                ...
                if records_processed % 50 == 0:
                    registry_session.commit()
                    _logger.info(f"Committed batch, processed {records_processed} records so far")
                    
            except Exception as e:
                _logger.error(f"Error processing vehicle record {rta_record.regnno}: {str(e)}")
...
def _update_vehicle_ownership_record(
    ...
    existing_record.registration_from_date = _parse_date_string(rta_record.regn_fromdate)
    ...
    existing_record.no_of_wheels = G2PRegistryVehicleOwnership.get_wheels_for_cov(rta_record.cov)
