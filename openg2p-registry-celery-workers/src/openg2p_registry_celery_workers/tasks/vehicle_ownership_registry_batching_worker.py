...
def vehicle_ownership_registry_batching_worker(id: int):
    ...
    with registry_session_maker() as registry_session:
        ...
        try:
            ...
            _logger.info(f"Batching Worker Task Payload JSON: {task_record.worker_payload}")
            ...
            _logger.info(f"Batching with params - issue_date: {registration_issue_date}, "
            ...
            registry_session.commit()
            _logger.info(f"Successfully created {batches_created} batch tasks for {total_records} total records")
            ...
            if task_record:
                ...
                registry_session.commit()
            _logger.error(f"Batching worker task failed for id: {id}, error: {str(e)}", exc_info=True)
            raise
